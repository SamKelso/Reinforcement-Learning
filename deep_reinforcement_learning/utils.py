...
def loss(policy_dqn:DQN, target_dqn:DQN,
    ...
    if(ddqn):
        policy_dqn_action_inds = policy_dqn(next_states).max(1).indices.reshape([-1,1])
        bellman_targets = (~dones).reshape(-1)*(target_dqn(next_states)).gather(1, policy_dqn_action_inds).reshape(-1) + rewards.reshape(-1)
    else:
